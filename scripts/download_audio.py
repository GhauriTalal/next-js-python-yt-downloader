...
def download_audio(url, format_id, output_path="downloads"):
    ...
    try:
        ...
        # Allow passing a cookies file path via env var `YTDL_COOKIES` or
        # as the third CLI argument when calling the script.
        ...
        # If a cookies file was provided, tell yt-dlp to use it.
    ...
    if len(sys.argv) != 3:
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "Usage: python download_audio.py <url> <format>",
                }
            )
        )
        sys.exit(1)
