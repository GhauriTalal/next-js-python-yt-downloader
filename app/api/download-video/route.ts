...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try running the Python script with a couple of common python executables
    ...
    // Try to parse stdout as JSON. yt-dlp sometimes emits progress/log lines
    // mixed with output; attempt to extract a JSON object from the stream.
    ...
    if (!result) {
      // If direct parse failed, attempt to find the first '{' and last '}' and parse that slice.
    ...
    // Parsing failed — include raw output and stderr to help debugging.
